...
AMOUNT_TRAINING_DATA = 4129  # Disregarding data augmentation
AMOUNT_VALIDATION_DATA = 5  # Disregarding data augmentation
...
MOODLE_SESSION = '4g82ll5jt86nu1sc9dug1okslp'
...
download_from_moodle('https://lernen.min.uni-hamburg.de/mod/resource/view.php?id=6404', 'project_training_data.zip',
                     unzip=True, unzip_to=TRAINING_DATA_FOLDER)

download_from_moodle('https://lernen.min.uni-hamburg.de/mod/resource/view.php?id=6405', 'project_validation_data.zip',
                     unzip=True, unzip_to=VALIDATION_DATA_FOLDER)

download_from_moodle('https://lernen.min.uni-hamburg.de/mod/resource/view.php?id=6406', 'project_testing_data.zip',
                     unzip=True, unzip_to=TESTING_DATA_FOLDER)
...
image_paths_training, fixation_paths_training = image_paths[:AMOUNT_TRAINING_DATA], fixation_paths[
                                                                                    :AMOUNT_TRAINING_DATA]
image_paths_validation, fixation_paths_validation = image_paths[
                                                    AMOUNT_TRAINING_DATA:AMOUNT_TRAINING_DATA + AMOUNT_VALIDATION_DATA], fixation_paths[
                                                                                                                         AMOUNT_TRAINING_DATA:AMOUNT_TRAINING_DATA + AMOUNT_VALIDATION_DATA]

len(image_paths_training), len(fixation_paths_training), len(image_paths_validation), len(fixation_paths_validation)

image_paths_testing = read_txt_lines(TESTING_DATA_FOLDER + 'test_images.txt', TESTING_DATA_FOLDER)

len(image_paths_testing)

image_paths_testing[0]
...
class FixationTestingDataset(FixationDataset):
    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs, fixation_filenames=kwargs["image_filenames"])

        pattern = re.compile('image-(\d+)\.png')
        self.ids = []

        for filename in self.image_filenames:
            end = filename.split('/')[-1]
            self.ids.append(pattern.match(end).group(1))

        assert len(self.images) == len(self.ids)

    def __getitem__(self, index):
        return {'image': self.images[index], 'id': self.ids[index]}
...
class FixationTransform():
    def __call__(self, sample):
        ...
        return {'image': self.after(self.image(self.before(image))),
                'fixation': self.after(self.fixation(self.before(fixation)))}
...
final_transform = torchvision.transforms.Compose(
    [RescalingTransform(), TensorTransform(), NormalizationTransform(), EncoderTransform()])
...
ds_testing = FixationTestingDataset(
    image_filenames=image_paths_testing,
    transform=final_transform
)

ds_testing_notransform = FixationTestingDataset(
    image_filenames=image_paths_testing,
)
...
class TrainingStatusDisplayer:
    ...
    def show(self, epoch_index, batch_index, training_losses, validation_losses, training_losses_epoch):
        ...
        global_progress = (epoch_index * self.batches_per_epoch + batch_index) / (
                    TRAINING_EPOCHS * self.batches_per_epoch)
        ...
        real_validation_losses, alternative_validation_losses = zip(*validation_losses) if len(
            validation_losses) >= 1 else [tuple(), tuple()]
        ...
        if not PRODUCTION_MODE:
            if len(training_losses_epoch) >= 2:
                ax[0].plot([l if l <= 1 else 1 for l in training_losses_epoch],
                           label='Training loss current epoch (by batch)');
                ax[0].legend()
...
if not PRODUCTION_MODE:
    ...
    sns.distplot(validation_results);
...
def get_testing_prediction(index):
    output = None

    with evaluating(net):
        with torch.no_grad():
            input = ds_testing[index]['image'].unsqueeze(0).to(device)
            output = torch.sigmoid(net(input))
            output = output.cpu().numpy()[0][0]

    return output, ds_testing[index]['id']


get_testing_prediction(0)[0].shape


def display_testing_example(index):
    if PRODUCTION_MODE: return

    output, id = get_testing_prediction(index)
    image = ds_testing_notransform[index]['image'] / 255

    fig, ax = plt.subplots(1, 2, figsize=(12, 6))
    ax[0].imshow(image, norm=None, vmin=0, vmax=1)
    ax[0].set_title('Input image')

    ax[1].imshow(output, norm=None, vmin=0, vmax=1)
    ax[1].set_title('Predicted fixation')
...
def write_output(index):
    output, id = get_testing_prediction(index)
    filename = OUTPUT_FOLDER + 'prediction-' + id + '.png'
    imageio.imwrite(filename, img_as_ubyte(output))


display_testing_example(9)

for i in range(len(ds_testing)):
    write_output(i)

"""## Check for missing testing files"""

# import os.path
# base = "testing_data/images/test/image-"

# for i in range(4134, 5167+1):
#   fname = base + str(i) + ".png"
#   if not os.path.isfile(fname): print(fname)